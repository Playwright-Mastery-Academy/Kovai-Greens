...
export default function Shop() {
  ...
  return <div className="shop-app"><div className="shop-topline">GROWN WITH CARE. MADE FOR EVERYDAY PLATES. <span>COIMBATORE</span></div>
    ...
    <main className="shop-main">{loadError ? <section className="shop-empty"><h1>We couldn’t load the store</h1><p role="alert">{loadError}</p><button className="shop-primary" onClick={load}>Try again</button></section> : !catalog ? <section className="shop-empty" role="status">Loading the greens…</section> : token ? <>
      ...
      <button className="shop-back" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { const updated = await request('orders/' + token); setReceipt(updated); setNotice(updated.paymentStatus === 'PAID' ? 'Payment confirmed by the farm.' : 'Status refreshed. Payment updates after the farm records your payment.'); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}><RefreshCw size={15}/> Refresh payment status</button></section>
      ...
      <label>Preferred delivery date<input type="date" required min={catalog.minDate} max={catalog.maxDate} name="deliveryDate" value={selectedDate} onChange={e => setDeliveryDate(e.target.value)}/></label><label>Delivery note <small>(optional)</small><textarea maxLength={500} rows={2} name="notes" defaultValue=""/></label>
      {error && <p className="shop-error" role="alert">{error}</p>}{invalid && <p className="shop-error" role="alert">Some packs are unavailable for this delivery date. Choose another date, reduce quantities, or remove them before continuing.</p>}
